...
const STYLES: Record<TriageLevel, { bg: string; text: string; dot: string; label: string }> = {
  ...
  azul: { bg: "bg-primary-container/20", text: "text-primary", dot: "bg-primary", label: "Azul" },
};
...
export default function TriageBadge({ triage }: { triage: Triage }) {
  ...
  return (
    <div
      className={`px-2 py-0.5 ${s.bg} ${s.text} text-[11px] font-bold rounded-full flex items-center gap-1 w-fit`}
