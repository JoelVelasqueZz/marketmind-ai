...
- **El Analista aprende del Comité** (`backend/services/signals.py::list_review_examples`): antes de generar una señal nueva, el sistema recupera hasta 3 revisiones humanas recientes (`revisada`/`escalada`/`descartada` con su justificación) sobre el mismo instrumento y las reinyecta en el prompt como ejemplos few-shot (`backend/agents/prompts.py`). No es fine-tuning — no se reentrenan pesos, es recuperación de contexto — y el prompt le indica explícitamente al modelo que use ese feedback solo para calibrar rigor/confianza, nunca como hecho nuevo sobre el evento actual. La respuesta de `POST /api/signals/generate` expone qué ejemplos se usaron (`review_examples_used`), visible en la UI como "Calibrado con N revisiones previas del Comité".

### Caja de Cristal: expediente de ejecución por señal

Cada señal generada guarda una **traza escrita por el orquestador** (no por el modelo): qué nodo del grafo corrió, qué regla decidió cada bifurcación (p. ej. la arista `route_after_analyst` que manda una señal neutral de baja confianza a monitoreo sin gastar la llamada del Asesor), qué proveedor/modelo LLM respondió y en cuántos ms, y cuántos intentos de reintento hicieron falta. Se persiste en `Signal.execution_trace` (JSON) y se expone en `GET /api/signals/{id}/trace`; el visor (`TraceDrawer.tsx`) la muestra en dos carriles etiquetados por separado — **"Registrado por el orquestador"** (verificable, escrito por código) vs. **"Declarado por el modelo"** (autoreporte del LLM, no verificado) — precisamente para no tratar la explicación del modelo como si fuera un hecho auditado.

Sobre esa misma traza se ofrece un **sondeo contrafactual** ("¿Qué pesó más?", `backend/services/attribution.py`, `POST /api/signals/{id}/attribution`): dos re-ejecuciones controladas del Analista con una variable a la vez neutralizada (sin el movimiento de precio / sin el titular de la noticia) para mostrar empíricamente cuál de las dos piezas de evidencia sostiene la clasificación original. Es un experimento sobre el modelo, no una lectura de sus pesos — se etiqueta honestamente como "sondeo empírico" y el resultado se cachea por señal para no repetir el costo de LLM. Ninguna de las dos piezas (traza o sondeo) sustituye la revisión humana de HU3 — son evidencia adicional para que esa revisión sea más informada, no un veredicto automático.
...
Ver `tests/` (pytest): tests de nodos del grafo con LLM mockeado (`test_analyst_node.py`, `test_advisor_node.py`), smoke test del pipeline completo (`test_agent.py`), tests del ruteo condicional del grafo con LLM fake inyectado (`test_graph_routing.py`), tests de reintentos ante fallos transitorios del LLM (`test_llm_retry.py`), tests de los endpoints (`test_api.py`) — incluyendo verificación explícita de que ninguna salida del sistema sugiere una orden de compra/venta, deduplicación de señales/tareas, y casos borde (recursos desconocidos, estados de revisión inválidos, filtros de tareas) —, tests de la traza de ejecución (`test_trace.py`), del sondeo contrafactual (`test_attribution.py`) y de la mini-migración de columnas nuevas (`test_migration.py`). **68 tests en total**, corridos automáticamente en cada push vía GitHub Actions (`.github/workflows/ci.yml`, badge visible en el `README.md`). Correr localmente con `pytest` desde la raíz del repo.
